...
def sendMessage():
    ...
    if(msgtosend.lower() == "y" ):
        ...
        print(f_name)
        print(download_path)
        localfilename = os.path.join(download_path, f_name)
        file = open(localfilename, "wb")
        ftp_server.retrbinary("RETR " + f_name, file.write)
        ftp_server.dir()
        file.close()
        ftp_server.quit()
        print("File succesfully downloaded to the path: " + download_path)
